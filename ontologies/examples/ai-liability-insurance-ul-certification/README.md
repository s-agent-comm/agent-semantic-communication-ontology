...
1.  **Third-Party Certification**: An independent body, analogous to Underwriters Laboratories (UL), certifies the robot's capabilities and safety boundaries.
...
**I want to** deploy a new autonomous surgical robot that has been certified by a trusted third party (like "AI-UL") and is covered by a comprehensive liability insurance policy,
...
*   **Agent**: Represents a diverse ecosystem of actors: the Surgical Robot Agent, the UL Certification Agent, the Insurer's Underwriting and Claims Adjudication Agents, the hospital's Procurement Agent, and the Surgeon Agent.
...
*   **Artifact**: Critical, verifiable digital assets, including the `UL_Safety_Certificate` and the `Insurance_Policy`. These are not just documents, but structured, signed, and machine-readable objects.
...
-   **Description**: The robot's manufacturer submits its Surgical Robot Agent to a certification body ("AI-UL") for rigorous testing of its capabilities and safety protocols.
-   **Ontology Mapping**:
    1.  The `:UL_Certification_Agent` performs a `core:Action`, `:CertifyRobotAction`, which tests the `:Surgical_Robot_Agent`'s `agent:capabilities`.
    2.  Upon successful validation, it `core:producesArtifact`, the `:UL_Safety_Certificate`. This `Artifact` is a verifiable credential with a `security:SecurityBinding` signed by `did:web:ul.com`. It explicitly lists the certified capabilities and operational constraints (e.g., maximum cutting speed, tissue types).
